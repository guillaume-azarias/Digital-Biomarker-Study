...
def prophet_fit(
    ...
    # make prediction. Note the frequency of the sampling period used for the dataframe
...
#     model.add_seasonality(name='weekly', period=7, fourier_order=15)
    ...
    name = re.sub("[']", '', str(device)) + '_' + str(parameter) + '_' + end + '_' + '_cps_' + str(
        changepoint_prior_scale) + '_fo_' + str(daily_fo) + '_' + str('{:02d}'.format(pred_duration)) + 'td'
    ...
    mape = mean_absolute_percentage_error(
        y_true=df_pred['actual'], y_pred=df_pred['preds'])
